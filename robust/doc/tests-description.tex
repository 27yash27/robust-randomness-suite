...
\item \texttt{-n number} determines how much data should be consumed by each call of the test function; the exact interpretation of this number depends on the test function; some tests ignore this value and decide themselves how many bytes they need; for some other tests only some values of this number are allowed [\texttt{{-}-ntest}];

\item  \texttt{-m number} modifies the behavior of some tests that use additional parameters [\texttt{{-}-modifier}];

\item \texttt{-o directory\_name} asks to create files with the list of real numbers (provided by the test function calls) that were submitted to the Kolmogorov--Smirnov two-sample test, and tells the name of the directory where these files should be created; the directory should not exist before the call. Every Kolmogorov--Smirnov value obtained by the test is accompanied by two files in this directory (that show both samples); one could understand the behavior of the test better by looking at these files [\texttt{{-}-output}];
...
\item \texttt{-k} requires the exact computation with multi-precision integers for Kol\-mo\-go\-rov--Smir\-nov two-sample $p$-value (instead of long double computation performed by default); can be useful for very large \texttt{-p} or \texttt{-q} options (say, exceeding $5000$) [\texttt{{-}-ksexact}];
...
Let $X$ be the sequence of $n$ bits to be analyzed. For each word $w$ of length at most $m$ we compute the number $\textit{count}\,[w]$ of occurrences of $w$ in $X$ (considered as a cyclic word); the sum of all $\textit{count}\,[w]$ for all words $w$ of given length equals $n$. The we compute $\psi^2[k] = (2^{k}/n)\sum_w (\textit{count}\,[w])^2 - n$ where the sum is taken over all strings $w$ of length $k$.
...
See~\cite{marsaglia-zaman} for the original description. (The \texttt{dieharder} implemenation deviates from the original description in a rather strange ways, see \cite[Section 7.1]{shen-romashchenko-robust}; we followed the original description.) The test consumes $2^{21}+1$ unsigned $32$-bit integers and generates $32-10+1=23$ values according to $23$ possible positions of $10$-bit substrings in a $32$-bit string. For each of $23$ positions we get a word of length $n=2^{21}+1$ in $2^{10}$-letter alphabet. We count $2$-letter words that are \emph{not} factors of this word, and convert this count to a  (presumably uniform) $p$-value assuming normal distribution with some parameters (mean about 141909.33 and $\sigma$ about $290.46$). The \texttt{-n} parameter should be $2097153$ (${}=2^{21}+1$), the \texttt{-d} parameter should be $23$. Uses about $8$ Mbytes per test value.

\item[5] \texttt{oqso [test\_oqso.c]}

The test is similar to \texttt{opso} and is described in the same paper. (Again the \texttt{dieharder} implementation does something rather different,  see \cite[Section 7.1]{shen-romashchenko-robust}) The difference is that the alphabet contains $2^5$ letters (factors of length $5$ at different positions in the input integers), and we count factors of length $4$ that are missing. Consumes $2^{21}+3$ integers and produces $28$ values. Approximation mean and variance are slightly different ($141909.60$ and $294.656$). The \texttt{-n} parameter should be $2097155$ (${}=2^{21}+3$), the \texttt{-d} parameter should be $28$. Uses about $8$ Mbytes per test value.
...
\end{figure}
Let us stress again that the approximation errors do not destroy the validity of test results when using two-sample robust tests.
...
\item{20} \texttt{monobit [test\_monobit.c]}

This test (implementing NIST frequency (monobit) test, \cite[Section 2.1]{nistsp800-22-1a}) considers the input as a sequence of bits. It reads $32n$ bits where $n$ is the parameter in \texttt{-n} option (constant $32$ is chosen for better alignment), and computes the number $m$ of ones among the bits read. The deviation $s=|2m-n|/\sqrt{n}$ is computed, and the p-value that corresponds to the normal approximation (of the absolute value) is returned. (Again for testing one could use directly the number of ones without taking absolute value and conversion to p-values, but we followed NIST and it can be useful for sanity checks.)

\item{21} \texttt{nist\_block} [test\_nist\_block.c]
This test (implementing NIST frequency block test, \cite[Section 2.2]{nistsp800-22-1a}) considers the input as a sequence of bits. The input sequence is divided into $n\ge1$ blocks (where $n$ is the value of \texttt{-n} option) of length $32m$ (where $m\ge 1$ is the value of \texttt{-m} option). Then the frequencies of ones $p_1,\ldots,p_n$ and the sum $s=\sum_i (p_i-1/2)^2$ are computed. This sum is then converted to (approximate) $p$-value as described in \cite[Section 2.2]{nistsp800-22-1a}: $p=\textit{igamc}\,(n/2,4\cdot 32m\cdot s/2)$.
...
\section{Comparison with other test suites}

\begin{longtable}{|c|>{\raggedright\arraybackslash}p{3cm}|l|>{\raggedright\arraybackslash}p{4cm}|}
% --- 1. Header for the very first page ---
%\caption{My Multi-page Table} \label{tab:my_table} \\
\hline
\textbf{Source} & \textbf{Test name} & \texttt{rtest}  & Comments\\
\hline
\endfirsthead

% --- 2. Header for all subsequent pages ---
\hline
\textbf{Source} & \textbf{Testname} & \texttt{rtest} & Comments\\
\hline
\endhead

% --- 3. Footer for pages that aren't the last ---
\hline
...
% --- 5. Your actual data goes here ---
Diehard & Birthday spacing & \texttt{-t 16}& see comments above \\ \hline
Diehard & Overlapping 5-per\-mu\-tat\-ions (OPERM5)& &\\ \hline
Diehard & Binary rank $31\times 31$ & &\\ \hline
Diehard & Binary rank $32\times 32$&  \texttt{-t 11}&\\ \hline
Diehard & Binary rank $6\times 8$& \texttt{-t 12}  &\\ \hline
Diehard & Bitstream & \texttt{-t 13}, \texttt{-t 14}& overlap / non-overlap version\\ \hline
Diehard & OPSO & \texttt{-t 4} & Overlapping Pairs Sparse Occurence \\ \hline
Diehard & OQSO & \texttt{-t 5} & Overlapping Quadruples Sparse Occurence\\ \hline
Diehard & DNA & &  \\ \hline
Diehard & Count the $1$s  in a stream of bytes& & \\ \hline
Diehard & Count the $1$s for specific bytes & & \\ \hline
Diehard & Parking lot & & \\ \hline
Diehard & Minimum distance (2D) & \texttt{-t 17} & see comments above \\\hline
Diehard & 3D spheres & & \\ \hline
Diehard & Squeeze & &  \\ \hline
Diehard & Overlapping sums & \texttt{-t 8} & included even if Dieharder does not recommend it (no problem for robust version)\\ \hline
Diehard & Runs & \texttt{-t 6}& follows description in Knuth vol. 2\\ \hline
Diehard & Craps & & \\ \hline
\hline
%%%%%
Dieharder & Birthdays & \texttt{-t 16}& see comments above\\ \hline %  -d 0
Dieharder & Overlapping permutations (OPERM5)& &  \\ \hline % -d 1
Dieharder & $32\times32$ binary rank& \texttt{-t 11} & \\ \hline %  -d 2
Dieharder & $6\times 8$ binary rank&  \texttt{-t 12}&\\ \hline %  -d 3
Dieharder & Bitstream & \texttt{-t 13}, \texttt{-t 14} &overlap / non-overlap version\\ \hline %  -d 4
Dieharder & OPSO &  \texttt{-t 4} $*$ & Overlapping Pairs Sparse Occurence \par $*$with some changes, see above\\\hline %  -d 5
Dieharder & OQSO & \texttt{-t 5} $*$ & Overlapping Quadruples Sparse Occurence \par $*$with some changes, see above\\ \hline %  -d 6
Dieharder & DNA & & \\ \hline %  -d 7
Dieharder & Count the $1$s  in a stream of bytes & & \\ \hline %  -d 8
Dieharder & Count the $1$s (byte) & &\\ \hline %  -d Dieharder & & & \\ \hline %  -d 9 
Dieharder & Parking lot  & &\\ \hline %  -d Dieharder & & & \\ \hline %  -d 10
Dieharder &  Minimum distance (2D)& \texttt{-t 18} & see comments above\\ \hline %  -d 11
Dieharder &  3D spheres & &\\ \hline %  -d 12
Dieharder &  Squeeze & &\\ \hline %  -d 13
Dieharder &  Overlapping sums&\texttt{-t 8} & included even if Dieharder does not recommend it (no problem for robust version)\\ \hline % -d 14
Dieharder &  Runs &\texttt{-t 6} & follows description in Knuth v.2 (while Dieharder deviates slightly) \\ \hline%  -d 15
Dieharder &  Craps & &\\ \hline %  -d 16
Dieharder &  Marsaglia and Tsang GCD& &\\ \hline %  -d 18
%Dieharder &  Marsaglia and Tsang Gorilla& &\\ \hline %  -d 19 is not in the dieharder -l list, not implemented?
Dieharder &  STS monobit & \texttt{-t 20}&\\ \hline %    -d 100
Dieharder & STS runs& & \\ \hline %  -d 101
Dieharder & STS serial (generalized)& & \\ \hline %  -d 102
Dieharder & RGB bit distribution& & \\ \hline %  -d 200 
Dieharder & RGB generalized minimum distance& & \\ \hline %  -d 201  
Dieharder & RGB permutations & & \\ \hline %  -d 202
Dieharder & RGB lagged sum& & \\ \hline %  -d 203
Dieharder & RGB Kolmogorov-Smirnov& & \\ \hline %  -d 204
Dieharder & DAB Byte Distribution& \texttt{-t 6}& \\ \hline %  -d 205
Dieharder & DAB DCT& & \\ \hline %  -d 206
Dieharder & DAB fill tree& & \\ \hline %  -d 207
Dieharder & DAB fill tree 2& & \\ \hline %  -d 208
Dieharder & DAB monobit 2& & \\ \hline %  -d 209

% in the list_tests.c there are  other options and also written (in the old list_test())
...
%%%%%%% NIST (STS-2.1.2)
NIST (STS)&Frequency & \texttt{-t 20}& \\ \hline %  STS 01
NIST (STS)&Block frequency&\texttt{-t 21}  &\\ \hline %  STS02 
NIST (STS)&Cumulative sums & & \\ \hline %  STS03
NIST (STS)&Runs & & \\ \hline %  STS04
NIST (STS)&Longest run of ones & & \\ \hline %  STS05 
NIST (STS)&Rank & & \\ \hline %  STS06 
NIST (STS)&Discrete Fourier transform &\texttt{-t 10} & includes later corrections (see above)\\ \hline %  STS07 
NIST (STS)&Nonperiodic template matching& & \\ \hline %  STS08
NIST (STS)&Overlapping template matching& & \\ \hline %  STS09
NIST (STS)&Universal statistical & & \\ \hline %  STS10 (Maurer) 
NIST (STS)&Approximate entropy & & \\ \hline %  STS11
NIST (STS)&Random excursions & & \\ \hline %  STS12
NIST (STS)&Random excursions variant & & \\ \hline %  STS13 
NIST (STS)&Serial & \texttt{-t 2} & \\ \hline %  STS14 
NIST (STS)&Linear complexity & & \\ \hline % STS15


\hline
\iffalse
%%%%%%%
TestU01 & scomp\_LinearComp& & \\ \hline % 
TestU01 & scomp\_LempelZiv& & \\ \hline % 
%
TestU01 & sentrop\_EntropyDiscOver& & \\ \hline % 
TestU01 & sentrop\_EntropyDiscOver2& & \\ \hline % 
%
TestU01 & sknuth\_Collision& & \\ \hline %
TestU01 & sknuth\_CollisionPermut & &\\ \hline %  \
TestU01 & sknuth\_CouponCollector& & \\ \hline %  
TestU01 & sknuth\_Gap& & \\ \hline %  
TestU01 & sknuth\_MaxOft& & \\ \hline %  
TestU01 & sknuth\_Permutation & &\\ \hline %  
TestU01 & sknuth\_Run & &\\ \hline %
TestU01 & sknuth\_RunIndep & &\\ \hline %
TestU01 & sknuth\_Serial & &\\ \hline %
TestU01 & sknuth\_SerialSparse & &\\ \hline %
TestU01 & sknuth\_SimpPoker& & \\ \hline %  
%
TestU01 & smarsa\_BirthdaySpacings& & \\ \hline % 
TestU01 & smarsa\_CAT& & \\ \hline % 
TestU01 & smarsa\_CATBits& & \\ \hline % 
TestU01 & smarsa\_CollisionOver & &\\ \hline % 
TestU01 & smarsa\_GCD& & \\ \hline % 
TestU01 & smarsa\_MatrixRank & &\\ \hline %  
TestU01 & smarsa\_Opso & &\\ \hline %  
TestU01 & smarsa\_Savir2& & \\ \hline % 
TestU01 & smarsa\_SerialOver & &\\ \hline %  
%
TestU01 & smultin\_Multinomial& & \\ \hline % 
TestU01 & smultin\_MultinomialBits& & \\ \hline % 
TestU01 & smultin\_MultinomialOver& & \\ \hline % 
TestU01 & smultin\_MultinomialBitsOver& & \\ \hline % 
%
TestU01 & snpair\_BickelBreiman & \\ \hline %
TestU01 & snpair\_ClosePairs & \\ \hline %  
TestU01 & snpair\_ClosePairsBitMatch & \\ \hline %   
TestU01 & snpair\_DistanceCP& & \\ \hline % 
%
TestU01 & sspacings\_AllSpacings& & \\ \hline % 
TestU01 & sspacings\_AllSpacings2& & \\ \hline % 
TestU01 & sspacings\_ScanSpacings& & \\ \hline % 
TestU01 & sspacings\_SumLogsSpacings& & \\ \hline % 
TestU01 & sspacings\_SumSquaresSpacings& & \\ \hline % 
%
TestU01 & sspectral\_Fourier1& & \\ \hline % 
TestU02 & sspectral\_Fourier1& & \\ \hline % 
TestU01 & sspectral\_Fourier3& & \\ \hline % 
%
TestU01 & sstring\_AutoCor& & \\ \hline % 
TestU01 & sstring\_HamminCorr& & \\ \hline % 
TestU01 & sstring\_HammingIndep & &\\ \hline % 
TestU01 & sstring\_HammingWeight& & \\ \hline % 
TestU01 & sstring\_HammingWeight2& & \\ \hline % 
TestU01 & sstring\_LongestHeadRun& & \\ \hline % 
TestU01 & sstring\_PeriodsInStrings& & \\ \hline %  
TestU01 & sstring\_Run& & \\ \hline % 
%
TestU01 & svaria\_AppearanceSpacings & &\\ \hline %  
TestU01 & svaria\_CollisionArgMax& & \\ \hline %  
TestU01 & svaria\_SampleCorr &  &\\ \hline %  
TestU01 & svaria\_SampleMean &  &\\ \hline %  
TestU01 & svaria\_SampleProd & & \\ \hline %  
TestU01 & svaria\_SumCollector& & \\ \hline %  
TestU01 & svaria\_SumLogs& & \\ \hline %  
TestU01 & svaria\_WeightDistrib& & \\ \hline %  
%
TestU01 & swalk\_RandomWalk1 & &\\ \hline % 
TestU01 & swalk\_RandomWalk1a & &\\ \hline %   
TestU01 & swalk\_VarGeoN& &\\ \hline %   
TestU01 & swalk\_VarGeoP& &\\ \hline %   
 \fi  
\end{longtable}
