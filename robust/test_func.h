...
typedef bool test_func(long double *value, unsigned long *hash, PRG gen,
...
//  param[3] is the amount of used data in some test-depending sense (if
//  applicable) param[4..] have test-dependent meaning real_param[0..] also have
//  test-dependent meaning debug causes printing additional information
...
// 3 dimension = a multiple of 3, at most 93 size number of ints processed
// [sts_serial.c]
...
// 6 dimension = 1, uses 3*dize nints [DAB Byte Distribution Test,
// dab_bytedistrib.c]
...
test_func osums;
// 8 dimension = 1, uses 2*size-1 nints [diehard_sums.c, rejected as incorrect
// by dieharder]

test_func ent_8_16;
// 9 dimension = 4 (ent8, chi8, ent16, chi16), uses size nints [ent ubuntu
// utility]

test_func fftest;
// 10 dimension = 1 (number of peaks converted assuming normal approximation)
// [NIST FFT test]

test_func rank32x32;
// 11 dimension =1 (chi-square value for the rank distribution of param[3]
// matrices 32*32 from 32*param[3] ints

test_func rank6x8;
// 12 dimension =25 (chi-square value for the rank distribution of 6*8 matrices
// obtained
//    in 25 positions from 6n random ints (25*param[3] matrices from 6*param[3]
//    ints)

test_func bitstream_o;
// 13 overlapped bitstream test; dimension =1, param[3] should be 2^21+19 =
// 2097171

test_func bitstream_n;
// 14 non-overlapped bitstream test; dimension =1, param[3] should be 20*2^21 =
// 41943040

test_func lz_split;
// 15 dimension = 1; recommended param[3]=31250; in this case conversion to
// uniform distibution happens
...
// 21 dimension = 1, paran[3]=number of blocks, param[4]=block size -- NIST
// section 2.2, 3.2
...
test_func dab_dct;
// 29 dimension = 1, [Dieharder DAB DCT test]

test_func dab_filtering;
// 30 dimension = 1, [Dieharder DAB filtering test]
...
//    instead of one-tailed gsl_cdf_ugaussian_P; param[3]=31250 for NIST standard
