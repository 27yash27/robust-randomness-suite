...
// Here one should provide references to all test functions and their short
// descriptions
test_function functions_list[] = {
    ...
    {dab_dct, "Dieharder DAB DCT test"},
    {dab_filtering, "Dieharder DAB filtering test"},
...
bool dummy_dimension(long double *value, unsigned long *hash, PRG gen,
  ...
  static int count = 0;
  for (int i = 0; i < param[2];
       i++) { // dimension=param[2] according to conventions
...
bool all_bytes(long double *value, unsigned long *hash, PRG gen, int *param,
...
bool all_16(long double *value, unsigned long *hash, PRG gen, int *param,
            double *real_param, bool debug) {
  assert(param[2] == 1); // this function returns one value
#define NUM_16 65536
  bool occur[NUM_16];
