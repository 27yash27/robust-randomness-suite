...
def main():
    ...
    for g in a.generators:
        ...
        # assess appends to its output file, so a stale file would silently
        # extend. Always start from a clean one, inside the STS tree because
        # the C code writes there.
        scratch = sts / OUTPUT_NAME
        if scratch.exists():
            scratch.unlink()

        stdin_recipe = f"{g}\n1\n0\n{streams}\n"
        cmd = [str(assess), str(CHUNK_BITS)]
        print(f"generator {g}: {name} ...", end="", flush=True)
        t0 = time.time()
        proc = subprocess.run(cmd, cwd=sts, input=stdin_recipe, text=True,
                              capture_output=True)
        elapsed = round(time.time() - t0, 1)
        (logs / f"gen{g}_{name}.stdout").write_text(proc.stdout)
        (logs / f"gen{g}_{name}.stderr").write_text(proc.stderr)

        # The patched assess returns 1 even on a fully successful generation,
        # because the bit-packing patch returns early instead of running the
        # statistical tests. So the exit code is recorded but the produced byte
        # count is what decides success. A missing completion marker is also
    ...
    (out / "provenance.json").write_text(json.dumps({
        ...
        "bit_packing": "least significant bit first per 32-bit word, as patched "
                       "into src/utilities.c; see NIST_STS_Modifications_Guide.md",
        "compiler": tool_version("gcc", "--version"),
        "openssl": tool_version("openssl", "version"),
        "python": sys.version,
