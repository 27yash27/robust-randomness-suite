...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Sweep rtest -t 27 (Diehard OPERM5)."
    )
    ...
    parser.add_argument(
        "--output-root",
        default=default_results_root / "sweeps_t27",
        type=Path,
...
def main() -> int:
    ...
    print("Python is driving rtest for test 27.")
    print("For -t 27, the script fixes the required settings: -d 1 -n 1000000.")
    print()

    for p in args.p_values:
        run_dir = output_root / f"t27_p{p:03d}"
        if run_dir.exists():
