...
DIMENSION = 1
N_VALUE = 24000
FLOAT_LINE_RE = re.compile(r"^\s*[-+]?(?:\d+(?:\.\d*)?|\.\d+)(?:[eE][-+]?\d+)?\s*$")


def parse_args() -> argparse.Namespace:
    repo_root = Path(__file__).resolve().parent.parent.parent
    default_results_root = repo_root / "My-robust-Test-Results" / "manual_sweeps"
    parser = argparse.ArgumentParser(description="Sweep rtest -t 25 (Parking lot test).")
    ...
    parser.add_argument(
        "--output-root",
        default=default_results_root / "sweeps_t25",
        type=Path,
...
def main() -> int:
    ...
    print("Python is driving rtest for test 25.")
    print("For -t 25, the script fixes the required settings: -d 1 -n 24000.")
    print()

    for p in args.p_values:
        run_dir = output_root / f"t25_p{p:03d}"
        if run_dir.exists():
    ...
    with summary_csv.open("w", newline="", encoding="ascii") as f:
        ...
        for record in records:
            writer.writerow(
                [record.get("p", ""), record.get("returncode", ""), record.get("pvalue", ""), record.get("output_dir", "")]
            )
