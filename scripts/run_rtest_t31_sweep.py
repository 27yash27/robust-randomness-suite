...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Sweep rtest -t 31 (Linear Complexity)."
    )
    ...
    parser.add_argument(
        "--output-root",
        default=default_results_root / "sweeps_t31",
        type=Path,
...
def main() -> int:
    ...
    print("Python is driving rtest for test 31.")
    print(f"For -t 31, the script fixes the required settings: -d 1 -n {N_VALUE} -m {M_VALUE}.")
    print()

    for p in args.p_values:
        run_dir = output_root / f"t31_p{p:03d}"
        if run_dir.exists():
