...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Sweep rtest -t 26 (Squeeze test).")
    ...
    parser.add_argument(
        "--output-root",
        default=default_results_root / "sweeps_t26",
        type=Path,
...
def main() -> int:
    ...
    print("Python is driving rtest for test 26.")
    print("For -t 26, the script fixes the required settings: -d 1 -n 0.")
    print()

    for p in args.p_values:
        run_dir = output_root / f"t26_p{p:03d}"
        if run_dir.exists():
