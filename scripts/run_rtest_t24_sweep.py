...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Sweep rtest -t 24 (Count the 1s for specific bytes)."
    )
    ...
    parser.add_argument(
        "--output-root",
        default=default_results_root / "sweeps_t24",
        type=Path,
...
def summarize(values: list[float]) -> dict[str, float]:
    ...
    n = len(ordered)
    return {"min": ordered[0], "median": ordered[n // 2], "max": ordered[-1]}
...
def main() -> int:
    ...
    print("Python is driving rtest for test 24.")
    print("For -t 24, the script fixes the required settings: -d 4 -n 0.")
    print()

    for p in args.p_values:
        run_dir = output_root / f"t24_p{p:03d}"
        if run_dir.exists():
