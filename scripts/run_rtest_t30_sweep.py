...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Sweep rtest -t 30 (Dieharder DAB filtering)."
    )
    ...
    parser.add_argument(
        "--output-root",
        default=default_results_root / "sweeps_t30",
        type=Path,
...
def main() -> int:
    ...
    print("Python is driving rtest for test 30.")
    print("For -t 30, the script fixes the required settings: -d 1 -n 4096.")
    print()

    for p in args.p_values:
        run_dir = output_root / f"t30_p{p:03d}"
        if run_dir.exists():
