...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Sweep rtest -t 29 (Dieharder DAB DCT)."
    )
    ...
    parser.add_argument(
        "--output-root",
        default=default_results_root / "sweeps_t29",
        type=Path,
...
def main() -> int:
    ...
    print("Python is driving rtest for test 29.")
    print("For -t 29, the script fixes the required settings: -d 1 -n 4096.")
    print()

    for p in args.p_values:
        run_dir = output_root / f"t29_p{p:03d}"
        if run_dir.exists():
